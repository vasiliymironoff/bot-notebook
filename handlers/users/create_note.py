...
from keyboard.default import get_category_keyboard
...
async def new_note(message: Message):
    ...
    await message.answer('Выберите категорию', reply_markup=get_category_keyboard(message.from_user.id))
...
async def enter_category(message: Message, state: FSMContext):
    ...
    await message.answer("Введите название записи", reply_markup=ReplyKeyboardRemove())
...
async def enter_photo(message: Message, state: FSMContext):
    ...
    print('Начинается добавление')
    await database.insert_note(state)
    await state.finish()
